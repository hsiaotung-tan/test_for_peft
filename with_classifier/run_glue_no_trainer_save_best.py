...
def parse_args():
    ...
    parser.add_argument(
        "--max_length",
        type=int,
        default=512, # 512 for base, 128 for large
        help=(
...
def main():
    ...
    if args.task_name is not None:
        ...
        raw_datasets = load_dataset("/home/cver4090/Project/DATA/GLUE", args.task_name)
    else:
    ...
    # NOTE: modify model
    vera_config = VeraConfig(r=args.rank)
    ...
    completed_steps = 0
    best_acc = 0.
    out_dir = Path(args.output_dir) / str(args.seed)
    for epoch in range(args.num_train_epochs):
        ...
        if args.output_dir is not None and (epoch+1) % args.per_save_freq == 0:
            ...
            # NOTE: with classifier saved, if you do not use, please comment it.
            classifier_params = {name: param for name, param in unwrapped_model.state_dict().items() if 'classifier' in name}
            unwrapped_model.save_pretrained(out_dir / str(epoch), save_function=accelerator.save)
            torch.save(classifier_params, out_dir / str(epoch) / 'classifier.pth')
        # best save
        if eval_metric['accuracy'] > best_acc:
            ...
            torch.save(classifier_params, out_dir / 'best' / 'classifier.pth')

    # with open(out_dir / 'logs' / 'log.txt', 'w') as f:
    #     logger.info(f"best : {eval_metric}")
    #     f.write(f"best evalidate: {eval_metric} \n")
    accelerator.wait_for_everyone()
    unwrapped_model = accelerator.unwrap_model(model)
    # NOTE: with classifier saved, if you do not use, please comment it.
    classifier_params = {name: param for name, param in unwrapped_model.state_dict().items() if 'classifier' in name}
    unwrapped_model.save_pretrained(out_dir / 'latest', save_function=accelerator.save)
    torch.save(classifier_params, out_dir / 'latest' / 'classifier.pth')
